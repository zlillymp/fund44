# Run Specific or All Score Card Groups Against the Application

## Endpoint Details

**Endpoint Name:** Run specific, or all, Score Card Groups against the application

**Description:** Execute specific or all score card groups against a given application.

**HTTP Method:** POST

**URL Path:** `/api/v2/applications/{application_id}/score_card_group/execute`
...
|-------|------|----------|-------------|
| score_card_group_ids | array | No | Array of specific score card group IDs to execute. If empty or omitted, all score card groups will be executed. |
...
```bash
curl --request POST \
  --url https://api.lendflow.com/api/v2/applications/{application_id}/score_card_group/execute \
