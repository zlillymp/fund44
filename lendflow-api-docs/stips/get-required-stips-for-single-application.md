...
**Endpoint Name:** Get required stips for single application

**Description:** Retrieve the required stipulations (STIPs) for a specific application.

**HTTP Method:** GET

**URL Path:** `/api/v2/applications/{application_id}/required`
...
|-------|------|-------------|
| data | object[] | Array of required stipulation objects. |
...
```bash
curl --request GET \
  --url https://api.lendflow.com/api/v2/applications/{application_id}/required \
