...
**Description:** Update an existing attribute in the system

**HTTP Method:** `PUT`

**URL Path:** `/api/v2/attributes/{id}`
...
|-----------|------|----------|-------------|
| `id` | string | Yes | The ID of the attribute to update. |
...
| `is_readonly` | boolean | No | Whether the attribute is read-only. |

### cURL Example

```bash
curl --request PUT \
  --url https://api.lendflow.com/api/v2/attributes/{id} \
